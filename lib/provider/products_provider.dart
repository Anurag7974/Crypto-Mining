...
class ProductsProvider with ChangeNotifier {
    ...
    await FirebaseFirestore.instance.collection('products').get().then((
      ...
      productSnapshot.docs.forEach((element) {
        _productsList.insert(0, ProductModel(
          ...
          income: element.get('income'),
          cycleDays: element.get('cycleDays'),
          salePrice: element.get('salePrice'),
          isOnSale: element.get('isOnSale'),
          isDouble: element.get('isDouble'),
        ));
      });
    });
    notifyListeners();
  ...
  List<ProductModel> findByCategory(String categoryName) {
    List<ProductModel> _categoryList = _productsList
        .where((element) => element.productCategoryName.toLowerCase().contains(categoryName.toLowerCase())).toList();
